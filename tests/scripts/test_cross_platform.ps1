...
$winIfDefs = Get-ChildItem $srcDir -Filter *.cpp -Recurse | Select-String "#ifdef _WIN32" | Measure-Object
$winEndIfs = Get-ChildItem $srcDir -Filter *.cpp -Recurse | Select-String "#ifdef _WIN32" -Context 0,50 | 
...
$audioFiles = Get-ChildItem $srcDir -Filter *.cpp
$leakedWin32 = $false
...
$filesystemUsage = Get-ChildItem $srcDir -Filter *.cpp | Select-String "std::filesystem" | Measure-Object
...
$atomicUsage = Get-ChildItem $srcDir -Filter *.h | Select-String "std::atomic|atomic<" | Measure-Object
Write-Test "Uses std::atomic for thread-safe operations" ($atomicUsage.Count -gt 0)

$mutexUsage = Get-ChildItem $srcDir -Filter *.h | Select-String "std::mutex|mutex " | Measure-Object
Write-Test "Uses std::mutex for synchronization" ($mutexUsage.Count -gt 0)

# Verify no platform-specific sleep calls
$sleepUsage = Get-ChildItem $srcDir -Filter *.cpp | Select-String "Sleep\(|sleep\(|usleep\(" | Measure-Object
...
$nominmaxUsage = Get-ChildItem $srcDir -Filter *.h -Recurse | Select-String "NOMINMAX" | Measure-Object
$nominmaxUsageAudio = Get-ChildItem $srcDir -Filter *.cpp -Recurse | Select-String "NOMINMAX" | Measure-Object
...
$pragmaOnce = Get-ChildItem $srcDir -Filter *.h -Recurse | Select-String "#pragma once" | Measure-Object
$headerCount = (Get-ChildItem $srcDir -Filter *.h -Recurse | Measure-Object).Count
...
$gccExtensions = Get-ChildItem $srcDir -Filter *.cpp -Recurse | Select-String "__attribute__|__builtin" | Measure-Object
...
$absoluteIncludes = Get-ChildItem $srcDir -Filter *.cpp | Select-String '#include\s+"[A-Z]:|#include\s+"/home|#include\s+"/usr' | Measure-Object
...
$backslashIncludes = Get-ChildItem $srcDir -Filter *.cpp | Select-String '#include.*\\' | Measure-Object
...
$fixedWidthTypes = Get-ChildItem $srcDir -Filter *.h | Select-String "uint\d+_t|int\d+_t" | Measure-Object
Write-Test "Uses fixed-width integer types" ($fixedWidthTypes.Count -gt 0)

# Check no assumptions about pointer sizes
$pointerCasts = Get-ChildItem $srcDir -Filter *.cpp | Select-String "reinterpret_cast<int>.*\*|reinterpret_cast<long>.*\*" | Measure-Object
